...
// نخزّن مرجع الجدول مرة واحدة لكل تشغيل بدل استدعاء
// ss_() في كل دالة (أسرع قليلاً).
let _ss = null;
function ss_() {
...
function addClass_(body) {
  const sheet = ss_().getSheetByName(SHEET_NAMES.CLASSES);
...
function addStudent_(body) {
  const sheet = ss_().getSheetByName(SHEET_NAMES.STUDENTS);
...
function addTeacher_(body) {
  const sheet = ss_().getSheetByName(SHEET_NAMES.TEACHERS);
...
function updateRecord_(body) {
  const sheet = ss_().getSheetByName(SHEET_NAMES.ATTENDANCE);
  const lastRow = sheet.getLastRow();
  const now = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm');
  const existing = lastRow >= 2 ? sheet.getRange(2, 1, lastRow - 1, 10).getValues() : [];
...
function addBehavior_(body) {
  const sheet = ss_().getSheetByName(SHEET_NAMES.BEHAVIORS);
  const id = newId_();
  const now = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm');
...
function addBehaviorType_(body) {
  const sheet = ss_().getSheetByName(SHEET_NAMES.BEHAVIOR_TYPES);
...
function addPoint_(body) {
  ...
  const sheet = ss_().getSheetByName(SHEET_NAMES.POINTS);
  const id = newId_();
  const now = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm');
